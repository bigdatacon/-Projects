    ...
    namespace serialization {
        ...
        uint32_t СalculateId(It start, It end, std::string_view name) {
            ...
            auto stop_it = std::find_if(start, end,
                                        [&name](const domain::Stop &stop) { return stop.stop_name == name; });
        ...
        TransportCatalogueSerialize(const transport_catalogue::TransportCatalogue &transport_catalogue) {
            ...
            for (const auto &bus: buses) {
                ...
                for (auto stop: bus.stops) {
                    uint32_t stop_id = СalculateId(stops.cbegin(),
                                                   stops.cend(),
                                                   stop);
            ...
            for (const auto &[pair_stops, pair_distance]: distances) {
                ...
                distance_proto.set_start(СalculateId(stops.cbegin(),
                                                     stops.cend(),
                                                     pair_stops.first->stop_name));

                distance_proto.set_end(СalculateId(stops.cbegin(),
                                                   stops.cend(),
        ...
        RenderSettingsSerialize(const transport_catalogue::RenderSettings &render_settings) {
            ...
            *render_settings_proto.mutable_underlayer_color_() = std::move(
                    ColorSerialize(render_settings.underlayer_color_));
        ...
        RenderSettingsDeserialize(const transport_catalogue_protobuf::RenderSettings &render_settings_proto) {
            ...
            render_settings.underlayer_color_ = ColorDeserialize(render_settings_proto.underlayer_color_());
            ...
            for (const auto &color_proto: render_settings_proto.color_palette_()) {
                render_settings.color_palette_.push_back(ColorDeserialize(color_proto));
            }
        ...
        void CatalogueSerialize(const transport_catalogue::TransportCatalogue &transport_catalogue,
            ...
            transport_catalogue_protobuf::TransportCatalogue transport_catalogue_proto = TransportCatalogueSerialize(
                    transport_catalogue);
            transport_catalogue_protobuf::RenderSettings render_settings_proto = RenderSettingsSerialize(
                    render_settings);
            transport_catalogue_protobuf::RouteSettings routing_settings_proto = RoutingSettingsSerialize(
                    routing_settings);
        ...
        Catalogue CatalogueDeserialize(std::istream &in) {
            ...
            return {TransportCatalogueDeserialize(catalogue_proto.transport_catalogue()),
                    RenderSettingsDeserialize(catalogue_proto.render_settings()),
                    RoutingSettingsDeserialize(catalogue_proto.routing_settings())};
        }
