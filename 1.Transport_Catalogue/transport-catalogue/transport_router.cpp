...
namespace transport_catalogue {
    namespace graph {
        ...
        ActivityProcessor::ActivityProcessor(transport_catalogue::TransportCatalogue &tc)
                : tc(tc) {
            graph_ = ::graph::DirectedWeightedGraph<double>(2 * tc.GetStopsQuantity());
            AddKnots();
        ...
        std::optional<DestinatioInfo>
        ActivityProcessor::GetRouteAndBuses(std::string_view stop_name_from, std::string_view stop_name_to) {
        ...
        void ActivityProcessor::AddStopsOneDirection(const std::deque<std::string_view> &stops,
            ...
            for (auto it = stops.begin(); std::next(it) != stops.end(); ++it) {
                ...
                ::graph::Edge<double> *edge_mirror = new ::graph::Edge<double>{num_vertex_1_wait, num_vertex1_go, tc.GetWaitTime(),
                                                             std::string(*it),
                                                             0};  // добавляю ребро зеркало для первой остановки
                ::graph::Edge<double> *edge_go_wait = new ::graph::Edge<double>{num_vertex1_go, num_vertex_next_wait, time_inner,
                                                              bus_name,
                                                              1};  // добавляю ребро зеркало для первой остановки
                ...
                for (auto it_inner = std::next(it); std::next(it_inner) != stops.end(); ++it_inner) {
                    ...
                    double time_min_1_2 = distance_1_2 / (tc.GetVelocity() * 1000 / 60) +
                                          sum_time; // увиличиваю время на то что уже было накоплено пока ехал до этой остановки

                    int span_count =
                            std::distance(stops.begin(), std::next(it_inner)) - std::distance(stops.begin(), it);

                    ::graph::Edge<double> *edge_go_wait_inner = new ::graph::Edge<double>{num_vertex1_go, num_vertex_inner_next_wait,
                                                                        time_min_1_2, bus_name, span_count};
        ...
        void ActivityProcessor::AddStopsNonRoundTrip(std::deque<std::string_view> stops,
                                                     const std::string &bus_name) {  // Тут если передать stops по константной ссылке то ошибка в utility
